...
import { AppState, HairstyleOption, FaceAnalysis, TransformationResult } from './types';
import { HAIRSTYLES, SCANNING_MESSAGES } from './constants';
...
const App: React.FC = () => {
  ...
  const handleApplyStyle = async (style: HairstyleOption) => {
    if (!capturedImage) return;
    setSelectedStyle(style);
    setAppState(AppState.TRANSFORMING);
    setStatusMessage(`Generating ${style.name}...`);
    try {
      const transformedImage = await transformHairstyle(capturedImage, style.prompt);
      setResult({ originalImage: `data:image/jpeg;base64,${capturedImage}`, transformedImage, style });
    ...
    <div className="relative h-[100dvh] w-full overflow-hidden bg-black font-sans select-none">
      ...
      {(appState === AppState.ANALYZING || appState === AppState.TRANSFORMING) && (
        <div className="absolute inset-0 z-30 flex flex-col items-center justify-center bg-black/60 backdrop-blur-sm px-10">
          ...
          <p className="text-white font-bold tracking-tight text-center">{statusMessage}</p>
        </div>
      )}
          ...
          {appState === AppState.IDLE && (
            <div className="flex flex-col items-center gap-10">
              <div className="text-center">
                <h1 className="text-3xl font-black mb-1">HairLab AI</h1>
                <p className="text-white/60 text-xs">Professional Real-time Preview</p>
              </div>
            ...
            <div className="space-y-6">
              ...
              <div className="flex gap-4 overflow-x-auto no-scrollbar py-2">
                {HAIRSTYLES.map((style) => (
                  <button key={style.id} onClick={() => handleApplyStyle(style)} className={`flex-shrink-0 flex flex-col items-center gap-2 ${selectedStyle?.id === style.id ? 'scale-105 opacity-100' : 'opacity-40'}`}>
                    <div className={`w-16 h-16 rounded-full flex items-center justify-center border-2 transition-all ${selectedStyle?.id === style.id ? 'bg-blue-600 border-white' : 'bg-white/10 border-white/10'}`}>
